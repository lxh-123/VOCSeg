...
def maybe_download_and_extract(hypes):
    ...
    # data_voc_zip = os.path.join(data_dir, 'VOC2012.zip')
    vgg_weights = os.path.join(data_dir, 'weights')
    data_voc_dir = os.path.join(data_dir, 'VOC2012')

    if os.path.exists(vgg_weights) and os.path.exists(data_voc_dir):
        logging.error("VOC2012 and model weights are provided.")
        return
    else:
        logging.error("VOC Data or VGG Data is not provided.")
        exit(1)
...
def _make_data_gen(hypes, phase, data_dir):
    ...
    for image, gt_image in data:
        if phase == 'val':
            yield image, utils.load_segmentation_mask(hypes, gt_image)[:, :, :-1]
        elif phase == 'train':
            image, gt_image = jitter_input(hypes, image, gt_image)
            yield image, utils.load_segmentation_mask(hypes, gt_image)[:, :, :-1]
            image, gt_image = jitter_input(hypes, np.fliplr(image), np.fliplr(gt_image))
            yield image, utils.load_segmentation_mask(hypes, gt_image)[:, :, :-1]
...
def resize_label_image_with_pad(hypes, image, label, image_height, image_width):
    shape = label.shape
    ...
    # background is assigned to class label: 0
    new_label = np.zeros([image_height, image_width, 3])
...
def resize_label_image(image, gt_image, image_height, image_width):
    ...
    gt_image = scipy.misc.imresize(gt_image, size=(image_height, image_width), interp='nearest')
...
def _read_processed_image(hypes, q, phase):
    ...
    if 'whitening' not in hypes['arch'] or hypes['arch']['whitening']:
...
def _processe_image(hypes, image):
    ...
    if augment_level > 0:
        ...
        image = tf.image.random_contrast(image, lower=0.75, upper=1.25)
    if augment_level > 1:
        image = tf.image.random_hue(tf.reshape(image, [shape[1], shape[2], shape[3]]), max_delta=0.15)
        image = tf.image.random_saturation(image, lower=0.5, upper=1.6)
        image = tf.expand_dims(image, 0)
...
def main():
    """main."""
    with open('../hypes/VOCSeg.json', 'r') as f:
        hypes = json.load(f)
