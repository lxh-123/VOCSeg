...
def get_color2class(hypes):
    ...
    return color2class_dict


def get_class2color(hypes):
    """
    Load dictionary which maps classes to colors as well as the default class.
    ...
    Returns
    -------
    dict: class2color_dict
        The dictionary `class2color_dict` maps  class labels (ints) to (R, G, B) tuples.
    """
    class2color_dict = {}
    for i, cl in enumerate(hypes['classes']):
        for color in cl['colors']:
        ...
        class2color_dict[i] = color

    return class2color_dict


def load_segmentation_mask(hypes, gt_image):
    """
    ...
    color2class_dict = get_color2class(hypes)

    # Create gt image which is a matrix of classes
    gt = np.zeros((gt_image.shape[0], gt_image.shape[1], num_classes+1), dtype=int)  # num_classes channel
    assigned = np.zeros((gt_image.shape[0], gt_image.shape[1]), dtype=int)

    for color, class_label in color2class_dict.items():
        affected_pixels = np.all(gt_image == color, axis=2)
        gt[:, :, class_label] = affected_pixels
        assigned += affected_pixels
    remaining_pixels = np.logical_not(assigned)

    gt[:, :, num_classes] = remaining_pixels
