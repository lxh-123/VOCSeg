...
def evalExp(hypes, gt_image, cnn_image):
    '''
    Does the basic pixel based evaluation!
    '''

    assert len(gt_image.shape) == 3, 'Wrong size of input prob map'
    assert len(cnn_image.shape) == 3, 'Wrong size of input prob map'

    head = hypes['arch']['weight']
    shape = gt_image.shape
    cnn_image_argmax = np.argmax(cnn_image, axis=2)
    gt_image_argmax = np.argmax(gt_image, axis=2)

    # pixel accuracy / mean accuracy / mean IoU / weighted mean IoU
    intersection_class = np.zeros(shape[2])
    unionsection_class = np.zeros(shape[2])
    pixel_class = np.zeros(shape[2])
    #cnn_image_class = np.zeros(shape[0],shape[1])

    for i in np.arange(shape[2]):
        cnn_image_class = np.all(cnn_image_argmax.reshape(shape[0], shape[1], 1) == i, axis=2)
        intersection_class[i] = np.sum(cnn_image_class * gt_image[:, :, i])
        pixel_class[i] = np.sum(gt_image[:, :, i])
        unionsection_class[i] = pixel_class[i] + np.sum(cnn_image_class) - intersection_class[i]

    return intersection_class, pixel_class, unionsection_class
...
def pxEval(total_intersection_class, total_pixel_class, total_unionsection_class):

    '''
    @param total_intersection_class: vector
    @param total_pixel_class: vector
    @param total_unionsection_class: vector
    '''

    assert len(total_intersection_class.shape) == 1, 'Wrong size of input prob map'
    assert len(total_pixel_class.shape) == 1, 'Wrong size of input prob map'
    assert len(total_unionsection_class.shape) == 1, 'Wrong size of input prob map'

    pixel_sum = np.sum(total_pixel_class)

    prob_eval_scores = dict()

    prob_eval_scores['PixelAcc'] = np.float32(np.sum(total_intersection_class))/pixel_sum
    prob_eval_scores['MeanPixelAcc'] = \
        np.mean(np.float32(total_intersection_class)/total_pixel_class)
    prob_eval_scores['MeanIoU'] = \
        np.mean(np.float32(total_intersection_class)/total_unionsection_class)
    prob_eval_scores['FwMeanIoU'] = \
        np.sum(np.float32(total_intersection_class*total_pixel_class)/total_unionsection_class)/pixel_sum

    # prob_eval_scores['PixelAcc'] = np.sum(total_intersection_class)
    # prob_eval_scores['MeanPixelAcc'] = np.sum(total_pixel_class)
    # prob_eval_scores['MeanIoU'] = np.sum(total_unionsection_class)
    # prob_eval_scores['FwMeanIoU'] = np.sum(total_unionsection_class)
